...
@section('main')
    ...
    <div class="card-header pb-0 text-left bg-transparent">
        <h3 class="font-weight-bolder text-info text-gradient">Edit Transaksi Pembelian</h3>
        <p class="mb-0">Silakan perbarui detail pembelian barang</p>
    </div>
    <div class="card-body">
        <form action="{{ route('purchase.update', $purchase->id) }}" method="POST" role="form">
            @csrf
            @method('PUT')
            ...
            <div class="row">
                <div class="col-md-6 mb-4">
                    ...
                    <div class="input-group">
                        ...
                        <input type="date" name="tgl_nota" value="{{ old('tgl_nota', $purchase->tgl_nota) }}" class="form-control form-control-lg border-radius-md" required>
                    </div>
                </div>
                    ...
                    <div class="input-group">
                        ...
                        <select name="id_distributor" class="form-select form-control-lg border-radius-md" required>
                            @foreach ($distributor as $dis)
                                <option value="{{ $dis->id }}" {{ (old('id_distributor', $purchase->id_distributor) == $dis->id) ? 'selected' : '' }}>
                                    {{ $dis->nama_distributor }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                ...
                @php
                    // Ambil detail pertama (asumsi satu transaksi di form ini mengedit satu jenis buku)
                ...
                <div class="col-md-6 mb-4">
                    ...
                    <div class="input-group">
                        ...
                        <select name="id_buku" id="id_buku" class="form-select form-control-lg border-radius-md" required>
                            <option value="">-- Cari Buku --</option>
                            @foreach ($books as $book)
                                <option value="{{ $book->id }}" 
                                    data-harga="{{ $book->harga_beli ?? 0 }}"
                                    {{ (old('id_buku', $detail->id_buku ?? '') == $book->id) ? 'selected' : '' }}>
                ...
                <div class="col-md-3 mb-4">
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" name="harga_beli" id="harga_beli"
                            value="{{ old('harga_beli', $detail->harga_beli ?? 0) }}"
                ...
                <div class="col-md-3 mb-4">
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" name="jumlah_beli" id="jumlah_beli"
                            value="{{ old('jumlah_beli', $detail->jumlah_beli ?? 0) }}"
                            class="form-control form-control-lg border-radius-md" placeholder="0" required>
                    </div>
                </div>
                ...
                <div class="col-md-12">
                    <div class="bg-gray-100 border-radius-lg p-3 d-flex justify-content-between align-items-center">
                        ...
                        <input type="hidden" name="total_bayar" id="total_bayar" value="{{ $purchase->total_bayar }}">
                    </div>
                </div>
            </div>
            ...
            <div class="d-flex justify-content-end mt-5">
                ...
                <button type="submit" class="btn bg-gradient-info btn-lg px-5 shadow-sm border-radius-lg text-white">
                    <i class="fas fa-save me-2 text-xs"></i> Update Transaksi
                </button>
            </div>
        </form>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Event listener saat buku diganti
        ...
        // Event listener saat harga atau jumlah diketik manual
        jumlahInput.addEventListener('input', hitungTotal);
        hargaInput.addEventListener('input', hitungTotal);

        // JALANKAN PERTAMA KALI agar data yang sudah ada langsung terhitung di total
        hitungTotal();
    });
</script>
@endsection
